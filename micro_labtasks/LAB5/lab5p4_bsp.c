...
void ADC_COMP_IRQHandler(void)
{
	...
	switch(state)
	{
		case play:
			...
			{
				TIM17->CCR1 = track_1[track_index];
				track_index++;
			}
			else
			{
	...
	{
		state = v; /* switch back to v is more than 5 seconds passed */
		track_index = 0;
	...
	{
		state = v; /* switch back to idle is more than 5 seconds passed */
		track_index = 0;
